...
bus_service = ServiceBusService(
    ...
    shared_access_key_name='SampleharedAccessKey',
    shared_access_key_value='Y89IZY0BwutpsCoOjgLR7/iUyWXrGtr+HKYPa3VX112c2=')  
...
def main():
    try:
        ...
        LogTenThousandEvents(event)

    except Exception as exception:
        print(exception)

#Transport 10k events.
def LogTenThousandEvents(event):
    global countOfEvents
    allEvents = []
    for batchList in range(50):
        allEvents.append(BuildEventsInBatch(event))

    asyncEventLoop = asyncio.get_event_loop()
    allEventTasks = []

    for batchCount in range(50):
        #start = time.time()
        eventTask = asyncio.ensure_future(LogEventsToQueue(allEvents[batchCount]))
        allEventTasks.append(eventTask)
        #end = time.time()
    
    print(str(currentStamp.now().time().replace(microsecond=0)) + ": Logging 10k Events..Please Wait.. ")
